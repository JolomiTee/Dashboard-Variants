            ...
            <div className='flex justify-between'>
                ...
                <div className="flex gap-3">
                    {period.map((item, index) => (
                        <button onClick={() => {setTimePeriod(item)}} key={index} className={`${item === timePeriod ? 'bg-hotels-black text-white' : 'text-black'} font-visuelt-pro flex gap-3 h-[36px] items-center justify-center px-3 rounded-4`}>{item}</button>
                    ))}
                </div>
